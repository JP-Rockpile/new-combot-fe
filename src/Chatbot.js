...
function Chatbot() {
    ...
    useEffect(() => { // This useEffect hook runs once when the component mounts
        const fetchInitialMessage = async () => {
            try {
                const response = await fetch('http://3.16.108.219/api/chatbot/initial/'); // Adjust this URL to your GET endpoint
                if (!response.ok) {
    ...
    const fetchClosingMessage = async () => {
        try {
            const response = await fetch('http://3.16.108.219/api/chatbot/closing/');
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const data = await response.json();
    ...
    const sendMessage = async (e) => {
        ...
        try {
            const response = await fetch('http://3.16.108.219/api/chatbot/', {
                method: 'POST',
                headers: {
